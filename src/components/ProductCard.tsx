...
export function ProductCard({ product }: { product: Product }) {
  ...
  const lowStock = product.stock <= 2 && product.stock > 0;
  ...
  return (
    <Link
      ...
      className={`group block iris-card ${ui.cardBase} ${ui.cardRadius} ${ui.cardHover}`}
    >
      <div className={`relative ${ui.productCardAspect} overflow-hidden bg-surface`}>
        {lowStock && (
          <span className="absolute top-3 left-3 bg-black/50 backdrop-blur-sm text-white text-[10px] font-bold px-2.5 py-1 rounded-full z-10">
            آخرین موجودی
          </span>
        )}
        {outOfStock && (
