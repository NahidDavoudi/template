/**
 * core/demoApi.js — پاسخ‌های mock برای حالت نمایشی (demoMode)
 */
import demoData from '../config/demo.data.js';
...
export function demoProductList(filters = {}) {
  ...
  if (filters.category) {
    items = items.filter((p) => p.category_slug === filters.category);
  }
...
export function demoProductGet(id) {
...
export function demoCategoryBySlug(slug) {
  return demoData.categories.find((c) => c.slug === slug) || null;
}

export function demoOrderCreate(data) {
